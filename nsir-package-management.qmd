...
This workshop assumes that you actively work in RStudio project environments. RStudio projects help with setting up your work environment for a separate projects with their own working directory, scripts, and history. This synergies particularly well with `{renv}`, which we will use for package management.

In this workshop, you should be in the `NSIR-package-management` project environment, as indicated by the top right portion of the R Studio window.

If not, you can open the project by navigating to the workshop directory in Explorer (for Windows) or Finder (for MacOS) and opening `R-package-management-workshop.Rproj`.

### Packages, Libraries, and Repositories

![](images/r_env.png){width="567"}

-   **Package**: A package is a standardized collection of material extending R functionalities, e.g. providing code, data, or documentation. You *interact* with packages. Each package has a version number associated with it. Packages can be distributed and shared with others.

-   **Library**: A place (directory) where R knows to find packages it can use. Where you *store* packages. There can be multiple libraries.
    ...
    -   By default, there are two libraries:

        -   **User library**: where packages are installed by the user

        -   **System library**: where base packages for R are installed
...
-   **Repository:** Primary vehicle for organizing and distributing R packages. Where you *install* packages from.
    ...
    -   The most important repository is **CRAN** (Comprehensive R Archive Network)

        -   Other repositories include [Bioconductor](https://bioconductor.org/), [Posit Public Package Manager](https://packagemanager.posit.co/client/#/), and [R Universe](https://r-universe.dev/search)
...
### How R finds packages (default behavior)
...
What is the path to the R library on your system? How many library paths are there?

```{r}

```

Which library is used by default when installing a new package? (Hint: check the documentation for `install.packages()` by running `?install.packages` . Maybe there's something about it in the `lib` argument)
...
What do you notice? We see a some messages printed on the console, a new `renv/` folder and renv lockfile (`renv.lock`).

We see that **.RProfile** is also created. **.RProfile** is run every time you start R in that project. `{renv}` uses this to configure library path for your project.
...
Which repositories are set up in your current session? Which one is the project library? What is its position in the library search paths?
...
Demo: Create a new script using other packages (i.e. `ggplot2`)
...
`{renv}`, on its own, is *not* a panacea. It solves only one part of the problem -- reproduction of a project's **R** package dependencies. There are a myriad of other factors that can affect the results of an analysis, including (but not limited to):gi

-   The version of **R**,git s
...
-   [Git introduction for R users](https://happygitwithr.com/)
-   [R Project workflow](https://r4ds.had.co.nz/workflow-projects.html)
